...
app.layout = html.Div(style={'font-family': 'Arial', 'max-width': '800px', 'margin': 'auto'}, children=[
    ...
    html.Div([
        ...
        dash_table.DataTable(
            id='matching-terms-table',
            columns=[{'name': col, 'id': col} for col in data.columns[1:11]],  # Columns 2 to 11
            data=[],
...
@app.callback(
    [Output('output-display', 'children'),
     Output('matching-terms-table', 'data')],
...
def update_output(selected_term):
    ...
    matching_terms_table_data = [data[data.iloc[:, 0] == selected_term].iloc[0, 1:11].to_dict()]
    ...
    return html.Div([
        ...
        html.Div([
            ...
            html.Div(match_text, style={**match_style, 'padding': '10px', 'border-radius': '5px', 'margin-top': '5px'})
        ], style={'display': 'flex', 'flex-direction': 'column', 'align-items': 'center'})
    ], style={'background-color': '#f4f4f4', 'padding': '20px', 'border-radius': '5px'}), matching_terms_table_data
